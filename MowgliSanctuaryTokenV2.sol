...
interface IRouter {
    ...
    contract MowgliSanctuaryTokenV1 is IERC20, Context {
    ...
    string private constant _name = "Mowgli Sanctuary Token";
    ...
    event setIncludeFromRewardTX(address TheFollowingAddressIsSetAsIncludeInReward);
    ...
    function initialize(address routerAddress) public initializer onlyMultiOwner {
        ...
        _tTotal = 90900000 * 10**uint256(_decimals);
        _rTotal = (MAX - (MAX % _tTotal));
        swapTokensAtAmount = 90_900 * 10**uint256(_decimals);
        maxBuyLimit = 9_09_000 * 10**uint256(_decimals);
        maxSellLimit = 9_09_000 * 10**uint256(_decimals);
        maxWalletLimit = 9_09_000 * 10**uint256(_decimals);
        genesis_block = block.number;
        deadline = 0;
        deadWallet = 0x000000000000000000000000000000000000dEaD;
        marketingWallet = 0x1C7F0c770E45030DA303728969821f800dd55476;

        launchtax =Taxes(0, 0, 0);
        taxes = Taxes(0, 0, 0);
    ...
    function addLiquidity(uint256 tokenAmount, uint256 bnbAmount) private {
        ...
        (bool success, ) = address(router).call{value: bnbAmount}(
            abi.encodeWithSignature("addLiquidityETH(address,uint256,uint256,uint256,address,uint256)", address(this), tokenAmount, 10, 10, owners[0], block.timestamp));
    ...
    function setSellTaxes(bool state,uint _trnxId) external onlyMultiOwner {
        if(state==true){
        sellTaxes = Taxes(2, 1, 2);
        }
        else{
    ...
    function setTaxes(bool state,uint _trnxId) external onlyMultiOwner {
        if(state == true){
        taxes = Taxes(2, 1, 2);
        }
        else{
